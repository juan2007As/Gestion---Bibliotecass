...
def verificar_consistencia_libros():
    ...
    cursor.execute("""
        SELECT l.id_libro, l.titulo, l.disponible,
               COUNT(CASE WHEN p.id IS NOT NULL AND p.fecha_devolucion_real IS NULL THEN 1 END) as prestamos_activos
        FROM Libro l
...
class Libro:
    ...
    def obtener_imagen_base64(self):
        ...
        # Asegurar que la imagen sea bytes
        if isinstance(self.imagen, str):
            try:
                imagen_bytes = self.imagen.encode('latin1')
            except:
                return None
        else:
        ...
        if imagen_bytes.startswith(b'\xFF\xD8\xFF'):
            ...
            mime_type = 'image/png'
        elif imagen_bytes.startswith(b'GIF87a') or imagen_bytes.startswith(b'GIF89a'):
            mime_type = 'image/gif'
        else:
            mime_type = 'image/jpeg'
    ...
    def to_dict(self):
        ...
        return {
            ...
            'estado_texto': 'Disponible' if self.disponible else 'No disponible',
            'imagen_url': self.obtener_imagen_base64() or '/static/imagenes/ImagenLibro.png'
        }
    ...
    def obtener_por_id(id_libro):
        ...
        conn.close()
        if fila:
            return Libro(id=fila[0], id_libro=fila[1], titulo=fila[2], autor=fila[3], editorial=fila[4], año=fila[5], genero=fila[6], disponible=bool(fila[7]), imagen=fila[8])
        return None
    ...
    def obtener_disponibles():
        ...
        conn = conectar_base_datos()
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM Libro WHERE disponible = 1')
        filas = cursor.fetchall()
        conn.close()
        return [Libro(id=f[0], id_libro=f[1], titulo=f[2], autor=f[3], editorial=f[4], año=f[5], genero=f[6], disponible=bool(f[7]), imagen=f[8]) for f in filas]
    
    @staticmethod
    def buscar_disponibles_por_termino(termino):
        """Busca libros disponibles por ID, título, autor o género"""
        conn = conectar_base_datos()
        cursor = conn.cursor()
...
class Usuario:
    ...
    def obtener_por_email(email):
        ...
        conn.close()
        if fila:
            # Orden correcto: id, nombre, apellido, documento, telefono, email, password, rol
            return Usuario(id=fila[0], nombre=fila[1], apellido=fila[2], documento=fila[3], telefono=fila[4], email=fila[5], password=fila[6], rol=fila[7] if len(fila) > 7 else "usuario")
        return None
    ...
    def obtener_todos():
        ...
        for f in filas:
            ...
            usuario = Usuario(
                ...
                password=f[6] if len(f) > 6 else None,
                rol=f[7] if len(f) > 7 else "usuario"
            )
    ...
    def obtener_por_documento(documento):
        ...
        if fila:
            ...
            return Usuario(
                ...
                password=fila[6] if len(fila) > 6 else None,
                rol=fila[7] if len(fila) > 7 else "usuario"
            )
        return None
...
class Prestamo:
    ...
    def obtener_con_info_libro(self):
        """retorna el prestamo con info del libro"""
        
        libro = libro = Libro.obtener_por_id(self.id_libro)
        if libro:
            self.titulo_libro = libro.titulo
            self.autor_libro = libro.autor
            self.genero_libro = libro.genero
            self.editorial_libro = libro.editorial
        return self
    ...
    def obtener_prestamos_activos_por_usuario(documento):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT p.*, l.titulo, l.autor, l.editorial, l.año, l.genero, u.nombre, u.apellido
            FROM Prestamo p
        ...
        for f in filas:
            ...
            prestamo.editorial_libro = f[9]
            prestamo.año_libro = f[10]
            prestamo.genero_libro = f[11]
            prestamo.nombre_usuario = f[12]
            prestamo.apellido_usuario = f[13]
...
def prestar_libro(id_libro, documento_usuario, dias_prestamo):
    ...
    # Verificar consistencia después del préstamo
    verificar_consistencia_libros()
    
    try:
        from app import email_service
        user_email = Usuario.obtener_por_documento(documento_usuario).email
        if user_email:
            email_service.send_loan_confirmation(
                user_email=user_email,
                user_name=f"{usuario[1]} {usuario[2]}",
                book_title=libro[1],
                due_date=fecha_devolucion_esperada.strftime("%Y-%m-%d")
            )
    except Exception as e:
...
def devolver_libro(id_prestamo):
    ...
    # Verificar consistencia después de la devolución
    verificar_consistencia_libros()
...
# ===============================
#   NOTIFICACIÓN DE MULTA (SI APLICA)
# ===============================
    if multa > 0:
        try:
            usuario_obj = Usuario.obtener_por_documento(documento_usuario)
            if usuario_obj and usuario_obj.email and '@' in usuario_obj.email:
                try:
                    from app import email_service
                    if email_service:
                        # Crear email de notificación de multa
                        subject = f"⚠️ Multa generada: ${multa}"
                        html_body = f"""
                        <div style="font-family: Arial, sans-serif; max-width: 600px;">
                            <h2 style="color: #dc3545;">⚠️ Multa por Retraso</h2>
                            <p>Hola <strong>{nombre} {apellido}</strong>,</p>
                            <p>Has devuelto el libro <strong>'{titulo}'</strong> con retraso.</p>
                            <div style="background: #fff3cd; padding: 15px; border-left: 4px solid #ffc107;">
                                <h3 style="color: #856404;">💰 Multa generada: ${multa}</h3>
                                <p>Días de retraso: {(datetime.now().date() - datetime.strptime(fecha_devolucion_esperada, '%Y-%m-%d').date()).days}</p>
                            </div>
                            <p>Por favor, acércate a la biblioteca para pagar la multa.</p>
                            <hr>
                            <p style="color: #6c757d; font-size: 12px;">Sistema de Biblioteca</p>
                        </div>
                        """
                    
                        email_service.send_email(usuario_obj.email, subject, html_body)
                        print(f"📧 Notificación de multa enviada a {usuario_obj.email}")
                except ImportError:
                    print("⚠️ Servicio de email no disponible para multa")
        except Exception as e:
            print(f"⚠️ No se pudo enviar notificación de multa: {e}")
...
def devolver_libros_multiples(ids_prestamos):
    ...
    try:
        ...
        # Verificar consistencia después de devoluciones múltiples
        verificar_consistencia_libros()
