...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function buscarLibros(termino, contenedorResultados, onSelect) {
        try {
            const response = await fetch(`/api/buscar_libros_admin?q=${encodeURIComponent(termino)}`);
            const libros = await response.json();
            
            if (libros.length === 0) {
                mostrarMensajeVacio(contenedorResultados, 'No se encontraron libros');
                return;
            }
    ...
    async function mostrarLibrosPrestados(usuario) {
        try {
            ...
            if (prestamos.error) {
                throw new Error(prestamos.error);
            }
            ...
            inicializarControlesMultiples(prestamos, usuario);
            ...
            librosPrespadosContainer.innerHTML = '<p>Error al cargar los préstamos. Inténtelo de nuevo.</p>';
    ...
    function mostrarResultadosLibros(libros, contenedor, onSelect) {
        ...
        libros.forEach(libro => {
            ...
            const clickableClass = libro.disponible ? 'clickable' : 'disabled';
            
            html += `
                <div class="resultado-item ${disponibleClass} ${clickableClass}" data-id-libro="${libro.id_libro}">
                    <div class="titulo">${libro.titulo}</div>
                    <div class="detalles">ID: ${libro.id_libro} • ${libro.autor} • ${libro.genero} (${libro.año})</div>
                    <div class="editorial">Editorial: ${libro.editorial}</div>
        ...
        // Agregar eventos de clic solo a libros disponibles
        contenedor.querySelectorAll('.resultado-item.clickable').forEach(item => {
            item.addEventListener('click', function() {
                const idLibro = parseInt(this.dataset.idLibro);
                const libro = libros.find(l => l.id_libro === idLibro);
                if (libro && libro.disponible && onSelect) {
                    onSelect(libro);
                }
            });
        });
